...
企业微信推送基于**群机器人 Webhook**，是一种单向通知机制。DotBot 通过 HTTPS POST 将消息投递到企业微信群，无需额外登录或客户端。
...
在 `appsettings.json` 中添加 `WeCom` 配置节：
...
**推荐做法**：将 `WeCom.WebhookUrl` 配置在全局配置（`~/.bot/appsettings.json`）而非工作区配置，避免将敏感信息提交到 Git 仓库。
...
企业微信机器人模式是 DotBot 的三种运行模式之一（CLI / QQ Bot / WeCom Bot）。启用后，DotBot 作为 HTTP 服务器接收企业微信的消息回调，并通过 AI Agent 自动回复。
...
5. 点击 **保存**，系统会验证 URL（DotBot 必须已启动）

### 2.3 配置

在 `appsettings.json` 中添加 `WeComBot` 配置节：
...
企业微信智能机器人 API 支持在单聊中接收语音和文件消息。DotBot 对这两种消息的处理方式如下：

#### 语音消息（接收）

企业微信会将语音消息自动转为文本（ASR），DotBot 接收到后会将转写文本作为文本输入交给 AI Agent 处理。即用户发送语音等价于发送文本消息，Agent 会正常回复。

消息流转：

```
用户发送语音 → 企业微信 ASR 转文本 → DotBot 接收 voice.content
  → 路由到 TextHandler → AI Agent 处理 → 回复文本
```

#### 文件消息（接收）

用户在单聊中发送文件时，DotBot 会接收到文件的加密下载 URL，并在通用消息处理器中记录。当前默认行为是记录文件 URL 并回复确认信息。
...
| 语音格式 | 仅支持 AMR 格式 |
| 语音转文本 | 由企业微信服务端完成，DotBot 直接获取转写结果 |
