...
DotBot provides two WeCom (WeChat Work) integration capabilities:
...
WeCom Push is based on **group bot Webhooks**, a one-way notification mechanism. DotBot delivers messages to WeCom groups via HTTPS POST, with no additional login or client required.
...
3. Enter a bot name (e.g., DotBot Notifications) and click **Add**
...
Add the `WeCom` config section in `appsettings.json`:
...
**Recommendation**: Place `WeCom.WebhookUrl` in global config (`~/.bot/appsettings.json`) rather than workspace config to avoid committing sensitive information to Git.
...
WeCom Bot mode is one of DotBot's four runtime modes (CLI / QQ Bot / WeCom Bot / API). When enabled, DotBot runs as an HTTP server receiving WeCom message callbacks and auto-replying via the AI Agent.
...
5. Click **Save**, the system will verify the URL (DotBot must already be running)

### 2.3 Configuration

Add the `WeComBot` config section in `appsettings.json`:
...
|-------------|------|----------|-------------|
| `Path` | string | Yes | Bot path (e.g., `/dotbot`) |
...
Session files are stored in the `.bot/sessions/` directory.
...
The WeCom Smart Bot API supports receiving voice and file messages in single chats. DotBot handles these two message types as follows:

#### Voice Messages (Receiving)

WeCom automatically converts voice messages to text (ASR). DotBot receives the transcribed text and passes it to the AI Agent as text input. This means sending a voice message is equivalent to sending a text message, and the Agent will respond normally.

Message flow:

```
User sends voice -> WeCom ASR transcription -> DotBot receives voice.content
  -> Routes to TextHandler -> AI Agent processes -> Replies with text
```

#### File Messages (Receiving)

When a user sends a file in a single chat, DotBot receives the file's encrypted download URL and logs it in the common message handler. The default behavior is to log the file URL and reply with a confirmation message.
...
| Voice-to-text | Done by WeCom server-side; DotBot directly receives the transcription |
