...
| DotBot | Compiled DotBot executable |
...
- `WS_URLS`: DotBot's WebSocket address (reverse WS)
...
## Step 2: Configure DotBot

Add QQ bot configuration in your DotBot workspace's `appsettings.json`:
...
```json
{
    ...
    "SystemInstructions": "You are DotBot, a concise and reliable QQ intelligent assistant.",
    "QQBot": {
        "Enabled": true,
        "Host": "0.0.0.0",
        "Port": 6700,
...
> If NapCat and DotBot run on the same machine, keep Host as `0.0.0.0`.
...
## Step 3: Configure NapCat to Connect to DotBot
...
6. Fill in configuration:
   - **URL**: `ws://127.0.0.1:6700/` (matching DotBot's Host:Port)
   - **Token**: If DotBot has AccessToken configured, enter the same value here
   - **Message Format**: Select `array` (**required**, do not select `string`)
7. Click **Save**

> If deploying NapCat via Docker, replace `127.0.0.1` in the URL with the IP of the machine running DotBot or the Docker network address.
...
- Session data is automatically saved in the DotBot workspace's `sessions/` directory
...
DotBot classifies QQ users into three roles:
...
When an admin triggers a Tier 2 operation, DotBot requests approval via QQ message:
...
# 3. Configure DotBot (ensure QQBot.Enabled = true in appsettings.json)
