...
public static partial class StreamAdapter
{
    ...
    {
        // Track CallId → (icon, name, argsJson) so that ToolCompleted events are self-contained
        // even when multiple tool calls run in parallel.
        var callIdMap = new Dictionary<string, (string Icon, string Name, string? ArgsJson)>();
            ...
            foreach (var content in update.Contents)
            {
                switch (content)
                {
                    ...
                    case FunctionCallContent functionCall:
                    {
                        ...
                        // Remember the mapping (including args) for the corresponding FunctionResultContent
                        if (!string.IsNullOrEmpty(functionCall.CallId))
                        {
                            callIdMap[functionCall.CallId] = (icon, title, argsJson);
                        }

                        yield return RenderEvent.ToolStarted(
                            icon,
                            title,
                            string.Empty,  // Content will be filled by thinking or result
                            argsJson);
                        break;
                    }
                    ...
                    case FunctionResultContent functionResult:
                    {
                        ...
                        yield return RenderEvent.ToolCompleted(
                            icon,
                            title,
                            argsJson ?? string.Empty,  // Content = args for display
                            result);  // AdditionalInfo = result
                        break;
                    }
                }
            }
