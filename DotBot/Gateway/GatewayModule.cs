...
/// </summary>
[DotBotModule("gateway", Priority = 100,
...
public sealed partial class GatewayModule : ModuleBase
{
    ...
    public override bool IsEnabled(AppConfig config)
        => config.QQBot.Enabled || config.WeComBot.Enabled || config.Api.Enabled || config.Acp.Enabled;
    ...
    public override IReadOnlyList<string> ValidateConfig(AppConfig config)
    {
        ...
        if (!config.QQBot.Enabled && !config.WeComBot.Enabled && !config.Api.Enabled && !config.Acp.Enabled)
            errors.Add("Gateway mode is enabled but no channel modules are enabled (QQBot, WeComBot, Api, Acp).");
        return errors;
    }
}
...
{
    /// <inheritdoc />
    public IDotBotHost CreateHost(IServiceProvider sp, ModuleContext context)
    {
