...
/// </summary>
public sealed class MemoryConsolidator(ChatClient chatClient, MemoryStore memoryStore)
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                AnsiConsole.MarkupLine("[grey][[Memory]][/] [yellow]Consolidation: LLM did not call save_memory, skipping.[/]");
                return;
            }
            ...
            foreach (var toolCall in completion.ToolCalls)
            {
                ...
                AnsiConsole.MarkupLine("[grey][[Memory]][/] [green]Consolidation complete.[/]");
                break;
            }
        }
        catch (Exception ex)
        {
            AnsiConsole.MarkupLine($"[grey][[Memory]][/] [red]Consolidation failed: {Markup.Escape(ex.Message)}[/]");
        }
    }
    ...
    public void ConsolidateInBackground(IReadOnlyList<AiChatMessage> messagesToArchive)
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            {
                AnsiConsole.MarkupLine($"[grey][[Memory]][/] [red]Background consolidation error: {Markup.Escape(ex.Message)}[/]");
            }
        });
    }
